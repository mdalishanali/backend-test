...
export class InviteUsersHelpers {
  ...
  public static updateInvite = async (inviteId: string, update) => {
    return await InvitedUsers.updateOne({ _id: inviteId }, update, {
      new: true,
    });
  };
  ...
  public static inviteSingleUser = async (
    ...
    const invitePromise = InvitedUsers.create(invitation);
    const emailPromise = emailService.inviteUserEmail(mailData);

    const [data, email] = await Promise.all([invitePromise, emailPromise]);
    return data;
  };
  ...
  public static findAllInvitedUser = async (query, user) => {
    ...
    let aggregatePipeline = [matchQuery, paginationQuery];
