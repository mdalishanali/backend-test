...
export const verifySocialLoginRegister = async ({
  oauth,
  accessToken,
  email,
}) => {
    ...
    switch (oauth) {
      case 'GOOGLE':
        const googleResponse = await fetch(
          `${config.GOOGLE_VERIFY_OAUTH_URL}?access_token=${accessToken}`
        );
        ...
        break;
      case 'FACEBOOK':
        const facebookFesponse = await fetch(
          `${config.FACEBOOK_VERIFY_OAUTH_URL}?access_token=${accessToken}`
        );
      ...
      case 'MICROSOFT':
        const microsoftResponse = await fetch(
          `${config.MICROSOFT_VERIFY_OAUTH_URL}`,
          {
            headers: {
...
export const giveReferralRewards = async (userId, points) => {
  try {
